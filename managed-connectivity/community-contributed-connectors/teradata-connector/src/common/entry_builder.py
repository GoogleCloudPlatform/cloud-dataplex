...
from src.name_builder import encode_non_ascii
...
class Columns(Enum):
    ...
    DECIMAL_TOTAL_DIGITS = 'DECIMAL_TOTAL_DIGITS'
    DECIMAL_FRACTIONAL_DIGITS = 'DECIMAL_FRACTIONAL_DIGITS'
    TABLE_CREATE_TIME = 'TABLE_CREATE_TIME'
    TABLE_LAST_ALTER_TIME = 'TABLE_LAST_ALTER_TIME'
...
def sanitize_column_name_udf(name: str):
    """Convert non-ASCII characters in column names to _u<codepoint>_ format.

    Dataplex import rejects non-ASCII characters in column names.
    Each non-ASCII character is replaced with _u<hex>_ using its
    Unicode code point (e.g. _u6D4B_, _u1F389_ for emoji). Column
    names with ASCII special characters (e.g. !@#$) are kept as-is
    — if Dataplex rejects them during import, that is a Dataplex
    constraint, not something the connector should silently alter.
    """
    return encode_non_ascii(name)
...
@F.udf(returnType=StringType())
def readable_type_name_udf(data_type: str):
    """Convert short code to human-readable type name."""
    return get_readable_type_name(data_type)
...
def build_dataset(config, df_raw, db_schema, entry_type):
    ...
    df = df_raw \
        .na.fill(value='UNKNOWN',subset=[Columns.DATA_TYPE.value]) \
        ...
        .withColumn(JSONKeys.METADATA_TYPE.value, choose_metadata_type_udf(Columns.DATA_TYPE.value)) \
        .withColumn(JSONKeys.DATA_TYPE.value, readable_type_name_udf(Columns.DATA_TYPE.value)) \
        .drop(Columns.DATA_TYPE.value) \
        .withColumnRenamed(Columns.COLUMN_NAME.value, JSONKeys.NAME.value) \
        .withColumn(JSONKeys.NAME.value, sanitize_column_name_udf(F.col(JSONKeys.NAME.value))) \
