...
* **Special characters in column names** -- Column names containing ASCII special characters (e.g., `!@#$%^&*{}|,?:;~`) are passed through as-is. The Dataplex import API may reject entries with these characters in schema field paths, resulting in `INVALID_UPDATE_ENTRY_REQUEST` errors. The connector does not alter these names to preserve the original metadata. Because the `dataplex-types.global.schema` aspect is required for `teradata-table` and `teradata-view` entry types, rejection of the schema aspect causes the entire entry to be rejected — affected tables/views will not be imported.
