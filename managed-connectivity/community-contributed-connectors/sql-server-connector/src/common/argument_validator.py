...
def validateArguments(parsed_args):
    ...
    if not parsed_args.target_location_id not in [GCP_REGIONS, 'global']:
        print(f"--target_location_id must be valid google cloud region or 'global' : {parsed_args.target_region_id}")
        sys.exit(1)
